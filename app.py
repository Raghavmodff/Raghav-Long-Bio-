...
# ====== GLOBAL SESSION FOR CONNECTION POOLING (IMPROVES SPEED) ======
...
def get_name_region_from_reward(access_token):
    try:
        ...
        # Using global session for connection reuse
        uid_res = session.get(uid_url, headers=uid_headers, verify=False)
...
def get_openid_from_shop2game(uid):
    ...
    try:
        ...
        res = session.post(openid_url, headers=openid_headers, json=payload, verify=False)
        data = res.json()
...
def perform_major_login(access_token, open_id):
    ...
    for platform_type in platforms:
        try:
            ...
            response = session.post(MAJOR_LOGIN_URL, data=edata, headers=LOGIN_HEADERS, verify=False, timeout=10)
...
def perform_guest_login(uid, password):
    ...
    try:
        resp = session.post(OAUTH_URL, data=payload, headers=headers, timeout=10, verify=False)
        data = resp.json()
...
def combined_bio_upload():
    ...
    response_data = {
        "Credit": "@Toxicffleaksss",
        "Telegram:": " @Toxicffleaksss",
...
if __name__ == "__main__":
    # Enable threading for handling multiple requests concurrently
